...
public class NeuralNetwork {
    ...
    string[] outputLabels = {"Step", "Left", "Right", "Stay"};
    float[] prevOutputs;
    public Layer[] layers;
        ...
        for (int n = 0; n < currentLayer.nodes.length; n++) {
          ...
          for (int c = 0; c < currentNode.connections.length; c++) {
            if(toggleTransparentNNConnections.getState()) {
              float opacity = abs(255*currentNode.connections[c]*layers[l-1].nodes[c].value/maxWeight);
              ...
              stroke(-currentNode.connections[c]*255, currentNode.connections[c]*255, 0, abs(currentNode.connections[c])*255/maxWeight);   // Max R/G when connection = -1 / 1
              strokeWeight(2);
            }
